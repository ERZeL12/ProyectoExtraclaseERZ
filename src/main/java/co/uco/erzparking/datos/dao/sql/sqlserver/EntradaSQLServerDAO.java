...
public class EntradaSQLServerDAO extends SQLDAO implements EntradaDAO {
	...
	public EntradaEntidad consultarPorId(final UUID id) {
		final String sql = "SELECT id, fechaHoraEntrada, vehiculo_id, servicio_id, operario_id FROM Entrada WHERE id = ?";
		try (PreparedStatement ps = getConexion().prepareStatement(sql)) {
	...
	public List<EntradaEntidad> consultarTodos() {
		final String sql = "SELECT id, fechaHoraEntrada, vehiculo_id, servicio_id, operario_id FROM Entrada";
		...
		try (PreparedStatement ps = getConexion().prepareStatement(sql);
			...
			throw ERZParkingExcepcion.crear(e, "Error al consultar todas las entradas", "SQLException al consultar tabla Entrada: " + e.getMessage());
		}
		return resultados;
	}
	...
	public List<EntradaEntidad> consultarPorFiltro(final EntradaEntidad filtro) {
		final StringBuilder sql = new StringBuilder("SELECT id, fechaHoraEntrada, vehiculo_id, servicio_id, operario_id FROM Entrada WHERE 1=1");
	...
	public EntradaEntidad consultarEntradaActivaPorVehiculo(final UUID vehiculoId) {
		final String sql = "SELECT e.id, e.fechaHoraEntrada, e.vehiculo_id, e.servicio_id, e.operario_id "
				+ "FROM Entrada e "
				+ "LEFT JOIN Salida s ON e.id = s.entrada_id "
				+ "WHERE e.vehiculo_id = ? AND s.id IS NULL";
		try (PreparedStatement ps = getConexion().prepareStatement(sql)) {
			...
			throw ERZParkingExcepcion.crear(e, "Error al consultar entrada activa del vehiculo", "SQLException al consultar entrada activa: " + e.getMessage());
		}
		return null;
	}
